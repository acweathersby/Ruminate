...
export default (function MarkDOM() {
    ...
    function node(type = 0, start = 0, end = 0, reduceUP = 0, reduceDN = 0, cap = 0, ignore = 0) {
        return { type, start, reduceUP, reduceDN, ignore, end, cap, children: [] }
    }
    ...
    function pNode(start) {
        return node(p, start, 0, p);
    }

    function txNode(start, end) {
        return node(tx, start, end);
    }

    function end(char, lex) { var i = 0; while (lex.ch == char) lex.next(), ++i; return i; }

    function space(lex) { return (lex.ty == lex.types.ws) ? (lex.next(), !0) : !1 }

    const md_headers = {
        "#": (lex, start, count) =>
            (count = end("#", lex), space(lex) && count < 7) ? node(2 << count, start + count) : pNode(start),
        ">": (lex, start) => node(bq, start, 0, bq),
        "`": function codeblock(lex, start) {
            const count = end("`", lex);
            return (count >= 3) ? node(cb, start, 0, 0, p | cb | br, cb) : pNode(start);
        },
        " ": function codeblock(lex, start) {
            const count = end(" ", lex);
            return (count >= 4) ? node(cb, start, 0, 0, p | cb | br, cb) : pNode(start);
        },
        "\n": function nl(lex, start) {
    ...
    function parseLineStart(lex, ch) {
        return (ch = lex.ch, md_headers[ch]) ? md_headers[ch](lex, lex.off) : pNode(lex.off);
    }
    ...
    function parseLine(lex, object, escape = "", escape_count = 0, start = lex.off) {
        ...
        while (!lex.END && lex.ty != lex.types.nl) {
            ...
            if (lex.ch == escape && escape_count == end(escape, lex.pk) + 1) {
                lex.sync();
                ...
                if (off - start >= 0)
                    setChild(object, txNode(start, off));
                ...
                case "_":
                    ...
                    if (count == 1)
                        ...
                        setChild(
                            object,
                            txNode(start, off),
                            parseLine(lex, node("bi", lex.off), ch, 3)
                        );
                    start = lex.off;
                    break;
                case "`":
        ...
        if (lex.off - start >= 1)
            setChild(object, txNode(start, lex.off));
            ...
            while (!lex.END) {
                ...
                if (output_stack.length > 0) {
                    const
                        ...
                        candidate = output_stack[index];

                    if (intermediate.reduceUP & candidate.type) {
                        setChild(intermediate, ...candidate.children);
                        output_stack[index] = intermediate;
                        break;
                    } else if (candidate.reduceDN & intermediate.type) {
                        setChild(candidate, ...intermediate.children);
                        candidate.reduceDN = (candidate.cap & intermediate.type) ? 0 : candidate.reduceDN;
                        break;
                    }
                }
            ...
            function r(d) {
                ...
                switch (d.type) {
                    ...
                    case tx:
                        ...
                        break;
                    case bold:
                        str += `<b>${d.children.map(r).join("")}</b>`
                        break;
                    case italic:
                        str += `<i>${d.children.map(r).join("")}</i>`
                        break;
                    case inline_code:
                        str += `<pre>${d.children.map(r).join("")}</pre>`
                        break;
                }
